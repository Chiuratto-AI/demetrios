...
impl ChangeDetector {
    ...
    pub fn new() -> Self {
        ChangeDetector {
            ...
            watch_patterns: vec![glob::Pattern::new("**/*.d").unwrap()],
...
mod tests {
    ...
    fn test_file_state() {
        ...
        let file_path = temp_dir.path().join("test.d");
    ...
    fn test_watch_patterns() {
        ...
        assert!(detector.should_watch(Path::new("src/main.d")));
        assert!(detector.should_watch(Path::new("lib/core.d")));
        assert!(!detector.should_watch(Path::new("test.rs")));

        detector.exclude("**/test/**").unwrap();
        assert!(!detector.should_watch(Path::new("src/test/foo.d")));
    }
    ...
    fn test_scan_directory() {
        ...
        fs::write(temp_dir.path().join("src/main.d"), b"module main").unwrap();
        fs::write(temp_dir.path().join("src/lib.d"), b"module lib").unwrap();
