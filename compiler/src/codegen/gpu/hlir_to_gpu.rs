...
//!
//! Every Knowledge value in Demetrios is lowered to a tuple of GPU values:
//!
//! ```text
//! Knowledge[T, ε, δ, Φ] ──> {
...
//!
//! ```ignore
//! use demetrios::codegen::gpu::{hlir_to_gpu, GpuTarget, PtxCodegen};
...
impl HlirToGpuLowering {
    fn new(config: LoweringConfig) -> Self {
        let module = GpuModule::new("demetrios_module", config.target);
        Self {
            config,
            module,
    ...
    fn lower_instruction(&mut self, block: &mut GpuBlock, instr: &HlirInstr) {
        ...
        match &instr.op {
            ...
            Op::PerformEffect { effect, op, args } => {
                let gpu_args: Vec<ValueId> = args.iter().map(|a| self.get_value(*a)).collect();
                let effect_func = format!("__demetrios_effect_{}_{}", effect, op);
