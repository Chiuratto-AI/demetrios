    ...
    pub fn diff(&self, var: &str) -> SymExpr {
        match self {
            ...
            SymExpr::Binary(left, op, right) => {
                ...
                match op {
                    BinOp::Add => {
                        SymExpr::Binary(Box::new(dl), BinOp::Add, Box::new(dr))
                    }
                    BinOp::Sub => {
                        SymExpr::Binary(Box::new(dl), BinOp::Sub, Box::new(dr))
                    }
                    BinOp::Mul => {
                        let term1 = SymExpr::Binary(
                            left.clone(),
                            BinOp::Mul,
                            Box::new(dr),
                        );
                        let term2 = SymExpr::Binary(
                            Box::new(dl),
                            BinOp::Mul,
                            right.clone(),
                        );
                    ...
                    BinOp::Div => {
                        let num1 = SymExpr::Binary(
                            Box::new(dl),
                            BinOp::Mul,
                            right.clone(),
                        );
                        let num2 = SymExpr::Binary(
                            left.clone(),
                            BinOp::Mul,
                            Box::new(dr),
                        );
                        let num = SymExpr::Binary(Box::new(num1), BinOp::Sub, Box::new(num2));
                        let denom = SymExpr::Binary(
                            right.clone(),
                            BinOp::Mul,
                            right.clone(),
                        );
            ...
            SymExpr::Unary(op, inner) => {
                ...
                let chain = match op {
                    ...
                    UnOp::Sqrt => {
                        ...
                        SymExpr::Binary(Box::new(SymExpr::Const(1.0)), BinOp::Div, Box::new(two_sqrt))
                    }
                    UnOp::Exp => {
                        SymExpr::Unary(UnOp::Exp, inner.clone())
                    }
                    UnOp::Log => {
                        SymExpr::Binary(
                    ...
                    UnOp::Sin => {
                        SymExpr::Unary(UnOp::Cos, inner.clone())
                    }
                    UnOp::Cos => {
                        SymExpr::Unary(UnOp::Neg, Box::new(SymExpr::Unary(UnOp::Sin, inner.clone())))
                    }
                    UnOp::Tan => {
                        let tan_f = SymExpr::Unary(UnOp::Tan, inner.clone());
                        let tan_sq = SymExpr::Binary(
                            Box::new(tan_f.clone()),
                            BinOp::Mul,
                            Box::new(tan_f),
                        );
                    ...
                    UnOp::Sinh => {
                        SymExpr::Unary(UnOp::Cosh, inner.clone())
                    }
                    UnOp::Cosh => {
                        SymExpr::Unary(UnOp::Sinh, inner.clone())
                    }
                    UnOp::Tanh => {
                        let tanh_f = SymExpr::Unary(UnOp::Tanh, inner.clone());
                        let tanh_sq = SymExpr::Binary(
                            Box::new(tanh_f.clone()),
                            BinOp::Mul,
                            Box::new(tanh_f),
                        );
                        SymExpr::Binary(
                    ...
                    UnOp::Abs => {
                        SymExpr::Call("sign".to_string(), vec![*inner.clone()])
                    }
                };
            ...
            SymExpr::Call(name, args) => {
                match name.as_str() {
                    ...
                    _ => {
                        SymExpr::Call(format!("d_{}", name), args.clone())
                    }
                }
            }
        }
        ...
        match self {
            SymExpr::Binary(left, op, right) => {
                ...
                match (op, &l, &r) {
                    ...
                    (BinOp::Pow, SymExpr::Const(a), SymExpr::Const(b)) => SymExpr::Const(a.powf(*b)),
