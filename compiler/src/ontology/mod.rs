//! Ontology Integration for Demetrios
//!
//! This module implements the 4-layer ontology architecture that enables
//! 15 million ontology terms to serve as native types in Demetrios.
//!
//! # Architecture
//!
//! ```text
    ...
    pub fn parse(input: &str) -> OntologyResult<Self> {
        ...
        if input.starts_with("http://") || input.starts_with("https://") {
            ...
            if input.contains("obolibrary.org/obo/") {
                if let Some(term) = input.rsplit('/').next() {
                    if let Some((prefix, local_id)) = term.split_once('_') {
                        return Ok(Self {
