...
impl Default for DownloadConfig {
    fn default() -> Self {
        Self {
            output_dir: PathBuf::from(".demetrios/ontology"),
            force: false,
            verify: true,
            timeout_secs: 60,
        }
            ...
            if let Some(ref mut c) = current {
                if let Some(id) = line.strip_prefix("id: ") {
                    ...
                    if let Some(start) = def.find('"') {
                        if let Some(end) = def[start + 1..].find('"') {
...
fn iri_to_curie(iri: &str, default_prefix: &str) -> String {
    ...
    if iri.contains("obolibrary.org/obo/") {
        if let Some(term) = iri.rsplit('/').next() {
            if let Some((prefix, local)) = term.split_once('_') {
    ...
    if let Some(fragment) = iri.split('#').last() {
        if fragment != iri {
