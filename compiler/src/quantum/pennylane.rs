...
//! Demetrios' epistemic semantics - solving the key gaps in Python Pennylane:
//!
//! ## Gaps in Pennylane (Python) that Demetrios Solves
//!
//! 1. **Runtime Python**: No compile-time checks (invalid gate order undetected)
//!    → Demetrios: Compile-time effect system validates circuit structure
//!
//! 2. **Basic Uncertainty**: Only sampling variance, no epistemic propagation
//!    → Demetrios: Full Beta posterior on parameters AND states
//!
//! 3. **No Provenance**: Circuits without trace (where did this param come from?)
//!    → Demetrios: Merkle tree of layers + params for auditable circuits
//!
//! 4. **Leaky Hardware Abstractions**: Good integration but error-prone
//!    → Demetrios: Refinement types for unitarity, fidelity bounds
//!
//! ## Key Innovation: Epistemic Quantum Circuits
//!
//! ```text
//! ┌─────────────────────────────────────────────────────────────────────┐
//! │              Pennylane-Native Demetrios Architecture                │
//! ├─────────────────────────────────────────────────────────────────────┤
//! │                                                                     │
//! │  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐ │
