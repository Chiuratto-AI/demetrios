//! REPL - Read-Eval-Print Loop for Demetrios
//!
...
impl Default for ReplConfig {
    fn default() -> Self {
        Self {
            ...
            show_types: true,
            use_jit: false,
            history_file: Some(".demetrios_history".to_string()),
        }
    }
}
...
impl Repl {
    ...
    fn print_help(&self) {
        println!("Demetrios REPL Commands:");
                        ...
                        match crate::check::check(&ast) {
                            Ok(hir) => {
                                ...
                                if let Some(item) = hir.items.last() {
                                    if let hir::HirItem::Function(f) = item {
