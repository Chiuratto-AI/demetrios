...
//!
//! ```rust
//! use demetrios::test::coverage::{CoverageTracker, CoverageConfig};
//!
...
impl CoverageReport {
    ...
    pub fn to_html(&self) -> String {
        ...
        html.push_str("<title>Demetrios Coverage Report</title>\n");
        html.push_str("<style>\n");
        html.push_str(
        ...
        html.push_str("<h1>Demetrios Coverage Report</h1>\n");
...
mod tests {
    ...
    fn test_file_coverage_line() {
        let mut cov = FileCoverage::new(PathBuf::from("test.d"));
        cov.total_lines = 10;
    ...
    fn test_coverage_merge() {
        ...
        data1.get_file_mut(Path::new("a.d")).hit_line(1);

        let mut data2 = CoverageData::new();
        data2.get_file_mut(Path::new("a.d")).hit_line(1);
        data2.get_file_mut(Path::new("a.d")).hit_line(2);

        data1.merge(&data2);

        let file = data1.files.get(Path::new("a.d")).unwrap();
    ...
    fn test_tracker_basic() {
        ...
        tracker.record_line(Path::new("test.d"), 1);
        tracker.record_line(Path::new("test.d"), 2);
        tracker.record_function(Path::new("test.d"), "main");

        tracker.stop_tracking();
        assert!(!tracker.is_active());

        let report = tracker.generate_report();
        assert!(report.data.files.contains_key(Path::new("test.d")));
    }

    #[test]
    fn test_lcov_output() {
        let mut data = CoverageData::new();
        let file = data.get_file_mut(Path::new("test.d"));
        file.total_lines = 10;
        file.hit_line(1);
        file.hit_line(2);
        ...
        assert!(lcov.contains("SF:test.d"));
    ...
    fn test_html_output() {
        let mut data = CoverageData::new();
        let file = data.get_file_mut(Path::new("src/main.d"));
        file.total_lines = 100;
        ...
        assert!(html.contains("src/main.d"));
    }
}
