...
mod tests {
    ...
    fn test_build_script_config_default() {
        ...
        assert_eq!(config.script, PathBuf::from("build.d"));
    ...
    fn test_parse_instruction_rerun_if_changed() {
        ...
        let inst = runner.parse_instruction("cargo:rerun-if-changed=src/lib.d");

        assert!(matches!(
            inst,
            Some(BuildInstruction::RerunIfChanged(p)) if p == PathBuf::from("src/lib.d")
        ));
    }
