# Demetrios Programming Guide for LLMs

This comprehensive guide enables LLMs to correctly generate Demetrios (D) code. Demetrios is a novel L0 systems + scientific programming language - it is NOT a dialect of Rust, Julia, Python, or any other language.
...
**IMPORTANT: D uses `&!` for exclusive/mutable references, NOT `&mut`**
...
Demetrios has first-class algebraic effects for tracking computational side effects.
...
Demetrios has built-in dimensional analysis for scientific computing.
...
Demetrios tracks knowledge, confidence, and provenance at the type level.
...
Demetrios supports C-compatible FFI through raw pointer types and extern blocks.
...
**CRITICAL: The following syntax/features do NOT exist in Demetrios L0:**
...
```d
// Both work in D, but be consistent:
fn foo() -> i32 {
    return 42    // OK
}
...
This guide is for **Demetrios L0** (low-level systems syntax).
