...
// Note: std::io and std::json not yet available in Demetrios stdlib
// This file requires IO and JSON support to function
...
pub fn validate_knowledge_file(path: &str) -> Result<ValidationReport, IoError> with IO {
    ...
    var failures: Vec<ValidationFailure> = vec![]
    ...
    while idx < lines.len() {
        ...
        match parse_json(line) {
            Ok(json) => {
                ...
                total_checks = total_checks + 7  // Base checks per record
                passed = passed + 7 - record_failures.len()
...
pub fn generate_report(report: &ValidationReport) -> string {
    ...
    if report.failed == 0 {
        ...
        let max_show = min(20, report.failures.len())
        var i: usize = 0
        while i < max_show {
            let failure = &report.failures[i]
...
pub fn main() with IO {
    ...
    // Note: println!/eprintln! don't exist in Demetrios
    // Use IO effect for output
    print("Validating: ")
    print(&input_path)
    print("\n")
    ...
    match validate_knowledge_file(&input_path) {
        ...
        Err(e) => {
            print(format("Error reading file: {}\n", e))
            exit(1)
        }
    }
}
