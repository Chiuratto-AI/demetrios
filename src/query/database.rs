...
pub struct QueryDatabase {
    ...
    storage: RwLock<HashMap<TypeId, Arc<dyn Any + Send + Sync>>>,
...
impl QueryDatabase {
    ...
    fn get_or_create_storage<K: QueryKey>(&self) -> Arc<QueryStorage<K>> {
        ...
        // Try to get existing storage
        {
            ...
            if let Some(s) = storage.get(&type_id) {
                if let Ok(typed) = Arc::clone(s).downcast::<QueryStorage<K>>() {
                    return typed;
                }
            }
        }
        ...
        let new_storage = Arc::new(QueryStorage::<K>::new());
        {
            ...
            storage.insert(type_id, Arc::clone(&new_storage));
        }
    ...
    fn get_storage<K: QueryKey>(&self) -> Option<Arc<QueryStorage<K>>> {
        ...
        storage
            .get(&TypeId::of::<K>())
            .and_then(|s| Arc::clone(s).downcast::<QueryStorage<K>>().ok())
    }
